        ...
        let identifyResultsVisible: (Bool) -> UIViewAnimations = { [weak self] (visible) in
            return {
                ...
                if visible {
                    ...
                    self.floatingPanelController = self.presentFloatingPanel(self.identifyResultsViewController)
                    self.floatingPanelController?.delegate = self
                }
                else {
                    // This will dismiss the floating panel controller.
